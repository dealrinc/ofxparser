...
//        $this->signupAccountInfo = $this->buildAccountInfo($xml->SIGNUPMSGSRSV1->ACCTINFOTRNRS);
    ...
    private function buildBankAccounts(SimpleXMLElement $xml)
    {
        // Loop through the bank accounts
        $bankAccounts = [];
        foreach ($xml->ACCTSTMT as $accountStatement) {
     ...
     * @throws \Exception
     */
    private function buildBankAccount(SimpleXMLElement $statementResponse)
    {
        $bankAccount = new BankAccount();
        $bankAccount->agencyNumber = $statementResponse->ACCTFROM->BRANCHID;
        $bankAccount->accountNumber = $statementResponse->ACCTFROM->ACCTID;
        $bankAccount->routingNumber = $statementResponse->ACCTFROM->BANKID;
        $bankAccount->accountType = $statementResponse->ACCTFROM->ACCTTYPE;

        $bankAccount->statement = new Statement();
        $bankAccount->statement->currency = "USD";// $statementResponse->CURDEF;

        $bankAccount->statement->startDate = Utils::createDateTimeFromStr(
            $statementResponse->STMTRS->DTSTART
        );

        $bankAccount->statement->endDate = Utils::createDateTimeFromStr(
            $statementResponse->STMTRS->DTEND
        );

        $bankAccount->statement->transactions = $this->buildTransactions(
            $statementResponse->STMTRS->STMTTRN
        );
